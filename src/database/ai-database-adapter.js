...
class AIDatabaseAdapter {
    ...
    try {
      ...
      if (this.useSchemaAware) {
        const schemaInitialized =
          await this.schemaAwareClassifier.initializeSchemaKnowledge();
        if (schemaInitialized) {
          console.log(
            "Schema-aware classification enabled with full ChromaDB support",
          );
        } else {
          console.log(
            "Schema-aware classification enabled with fallback mode (ChromaDB unavailable)",
          );
        }
      }
      ...
      const sanitizedError = securityConfig.sanitizeForLogging
        ...
        : error.message;
      console.error(
        "Failed to initialize AI Database adapter:",
        sanitizedError,
      );
  ...
  async processBusinessRequest(
    ...
    try {
      ...
      if (this.useSchemaAware) {
        ...
        intent = await this.schemaAwareClassifier.classifyIntent(
          userRequest,
          user.role || "employee",
        );
        console.log(
          "Schema-aware intent classified:",
      ...
      // Check for empty or invalid SQL before execution
      if (!sql || sql.trim().length === 0) {
        throw new Error("SQL generation failed - empty response from model");
      }
      ...
      if (this.useSchemaAware && !intent) {
        console.log(
          "Schema-aware classification failed, trying legacy approach",
        );
        try {
          intent = await intentClassifier.classifyIntent(
            userRequest,
          ...
          sql = await sqlGenerator.generateSQL(
            intent,
            null,
            null,
            ollamaClient,
  ...
  generateSummary(intent, results) {
    ...
    const filters =
      intent.filters && intent.filters.length > 0
        ? ` with filters: ${intent.filters.join(", ")}`
        : "";
    const limitText = intent.limit
      ? ` (showing top ${Math.min(intent.limit, count)})`
      : "";
  ...
  async fallbackProcessing(userRequest, user, originalError) {
    ...
    try {
      ...
      const result = await fallbackAdapter.processBusinessRequest(
        userRequest,
        user,
      );
  ...
  getAnalytics() {
    ...
    const avgProcessingTime =
      this.queryHistory.reduce((sum, entry) => sum + entry.processing_time, 0) /
      totalQueries;
    const avgConfidence =
      this.queryHistory.reduce((sum, entry) => sum + entry.confidence, 0) /
      totalQueries;
    ...
    return {
      ...
      most_requested_entity: Object.keys(entityCounts).reduce((a, b) =>
        entityCounts[a] > entityCounts[b] ? a : b,
      ),
      most_common_intent: Object.keys(intentCounts).reduce((a, b) =>
