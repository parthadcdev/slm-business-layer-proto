...
class APIFunctions {
  ...
  validateServiceOperation(service, operation) {
    ...
    if (!this.allowedOperations[service].operations.includes(operation)) {
      throw new Error(
        `Operation '${operation}' is not allowed for service '${service}'`,
      );
    }
  }
    ...
    switch (rule.type) {
      case "string":
        ...
        if (rule.minLength && value.length < rule.minLength) {
          throw new Error(
            `Parameter '${key}' must be at least ${rule.minLength} characters`,
          );
        }
        if (rule.maxLength && value.length > rule.maxLength) {
          throw new Error(
            `Parameter '${key}' must be no more than ${rule.maxLength} characters`,
          );
        }
      ...
      case "number":
        ...
        if (rule.max !== undefined && num > rule.max) {
          throw new Error(
            `Parameter '${key}' must be no more than ${rule.max}`,
          );
        }
        return num;
      ...
      case "array":
        ...
        if (rule.minItems && value.length < rule.minItems) {
          throw new Error(
            `Parameter '${key}' must have at least ${rule.minItems} items`,
          );
        }
        if (rule.maxItems && value.length > rule.maxItems) {
          throw new Error(
            `Parameter '${key}' must have no more than ${rule.maxItems} items`,
          );
        }
        return value;
