...
class ChunkProcessor {
  ...
  findNaturalBreakPoint(text) {
    ...
    const latestSentenceBreak = Math.max(
      ...sentenceBreaks.filter((pos) => pos > text.length * 0.6),
    );
    if (latestSentenceBreak > 0) {
      return (
        latestSentenceBreak + (text[latestSentenceBreak + 1] === "\n" ? 2 : 1)
      );
    }
  ...
  extractEntities(text) {
    const entities = {
      emails: this.extractPattern(
        text,
        /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g,
      ),
      phones: this.extractPattern(
        text,
        /\b\d{3}-\d{3}-\d{4}\b|\b\(\d{3}\)\s*\d{3}-\d{4}\b/g,
      ),
      dates: this.extractPattern(
        text,
  ...
  calculateReadability(text) {
    ...
    const score =
      206.835 - 1.015 * avgWordsPerSentence - (84.6 * avgCharsPerWord) / 100;
  ...
  mergeSmallChunks(chunks, minSize = this.minChunkSize) {
    ...
    for (const chunk of chunks) {
      if (!currentChunk) {
        ...
        currentChunk.metadata.merged_chunks =
          (currentChunk.metadata.merged_chunks || 1) + 1;
      } else {
        // Start new chunk
