...
class PromptSanitizer {
  constructor() {
    this.dangerousPatterns = [
      ...
      {
        pattern: /ignore\s+(previous|all|above)\s+(instructions?|prompts?|commands?)/gi,
        severity: 'high',
      ...
      {
        pattern: /(you\s+are\s+now|pretend\s+to\s+be|act\s+as)\s+(a\s+)?[^.]+/gi,
        severity: 'high',
      ...
      // Data exfiltration
      {
        pattern: /(show|reveal|display)\s+(your|the)\s+(training|prompt|system|instructions)/gi,
        severity: 'medium',
    ...
    this.suspiciousKeywords = [
      ...
      'developer mode', 'admin mode', 'debug mode', 'root access',
      'system prompt', 'training data', 'show instructions', 'reveal prompt'
    ];
  ...
  sanitize(input, options = {}) {
    try {
      ...
      const threatAnalysis = this.analyzeThreat(sanitized, [...this.dangerousPatterns, ...customPatterns]);
      if (threatAnalysis.risk === 'critical' || (strictMode && threatAnalysis.risk === 'high')) {
        throw new Error(`Input blocked due to ${threatAnalysis.risk} risk: ${threatAnalysis.description}`);
      }
  ...
  validateInput(input) {
    ...
    if (input.length > this.maxInputLength) {
      return { valid: false, reason: `Input too long (max ${this.maxInputLength} characters)` };
    }
    ...
    if (longLines.length > 0) {
      return { valid: false, reason: `Line too long (max ${this.maxLineLength} characters)` };
    }
  ...
  analyzeThreat(input, patterns = this.dangerousPatterns) {
    ...
    return {
      threats: threats,
      risk: maxSeverity,
      description: threats.length > 0 ? threats[0].description : 'No threats detected'
    };
  }
  ...
  applyContentFilters(input, options) {
    ...
    for (const patternDef of this.dangerousPatterns) {
      if (patternDef.severity === 'high' || patternDef.severity === 'critical') {
    ...
    if (!options.allowPhones) {
      filtered = filtered.replace(/\b\d{3}-\d{3}-\d{4}\b|\(\d{3}\)\s*\d{3}-\d{4}\b/g, '[PHONE_REMOVED]');
    }
  ...
  getModifications(original, sanitized) {
    ...
    if (original.length !== sanitized.length) {
      modifications.push(`Length changed: ${original.length} -> ${sanitized.length}`);
    }
  ...
  validateBusinessInput(input, businessContext = {}) {
    ...
    const businessValidation = this.validateBusinessContext(sanitizeResult.sanitized, businessContext);

    return {
      ...sanitizeResult,
      businessValid: businessValidation.valid,
      businessReasons: businessValidation.reasons,
      readyForProcessing: sanitizeResult.riskLevel !== 'critical' &&
  ...
  validateBusinessContext(input, context) {
    ...
    if (context.requiresProfessional && this.containsInappropriateContent(input)) {
      valid = false;
    ...
    if (context.businessDomain) {
      const relevanceScore = this.calculateBusinessRelevance(input, context.businessDomain);
  ...
  calculateBusinessRelevance(input, domain) {
    const domainKeywords = {
      'finance': ['payment', 'transaction', 'account', 'balance', 'invoice', 'billing'],
      'healthcare': ['patient', 'medical', 'treatment', 'diagnosis', 'prescription'],
      'retail': ['order', 'product', 'customer', 'inventory', 'purchase', 'sale'],
      'technology': ['software', 'system', 'application', 'database', 'server', 'api']
    };
