...
class SchemaAwareIntentClassifier {
    constructor() {
        ...
        this.intentPatterns = {
            ...
            analyze: ['analyze', 'analysis', 'insights', 'trends', 'metrics', 'performance'],
            count: ['count', 'how many', 'number of', 'total'],
            filter: ['filter', 'where', 'with', 'having', 'condition'],
            aggregate: ['sum', 'average', 'max', 'min', 'total', 'highest', 'lowest', 'most', 'least']
        };
    ...
    async classifyIntent(userRequest, userRole = 'employee') {
        try {
            ...
            const confidence = this.calculateConfidence(schemaContext, intent, userRequest);

            // Step 6: Extract additional query parameters
            const queryParams = this.extractQueryParameters(userRequest, filteredSchema);
            ...
            const result = {
                ...
                userRole,
                userRequest,
                fallbackReason: confidence < this.confidenceThreshold ? 'low_confidence' : null
            };

            console.log(`[SchemaAwareIntentClassifier] Request: "${userRequest}" → Entity: ${entity}, Intent: ${intent}, Confidence: ${confidence}`);
            return result;

        } catch (error) {
            console.error('[SchemaAwareIntentClassifier] Error in classification:', error);
    ...
    calculateConfidence(schemaContext, intent, userRequest) {
        ...
        const intentMatches = intentWords.filter(word => request.includes(word)).length;
        if (intentMatches > 0) {
            confidence += 0.2 * Math.min(intentMatches, 2); // Max 0.4 bonus
        }

        // Entity clarity bonus
        const hasSpecificEntity = ['categories', 'customers', 'products', 'inventory', 'suppliers']
    ...
    extractQueryParameters(userRequest, schemaContext) {
        ...
        const categoryMatch = request.match(/in the (\w+) category|(\w+) category|for (\w+)/);
        if (categoryMatch) {
        ...
        const knownCategories = ['gaming', 'electronics', 'computers', 'phones', 'audio', 'home', 'office'];
    ...
    async initializeSchemaKnowledge() {
        try {
            ...
            console.log('[SchemaAwareIntentClassifier] Schema knowledge base initialized');
            return true;
        } catch (error) {
            console.error('[SchemaAwareIntentClassifier] Failed to initialize schema knowledge:', error);
            console.log('[SchemaAwareIntentClassifier] Continuing with fallback classification mode');
            return false;
        }
    }
    ...
    getClassificationStats() {
        return {
            ...
            supportedEntities: ['orders', 'customers', 'products', 'categories', 'inventory', 'suppliers']
        };
    }
}
