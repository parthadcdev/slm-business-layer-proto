...
class SecurityConfig {
  ...
  loadSecurityConfig() {
    const config = {
      ...
      ai: {
        ...
        maxResponseLength: parseInt(process.env.AI_MAX_RESPONSE_LENGTH) || 16384,
  ...
  getJWTSecret() {
    ...
    if (secret === 'default-secret' || secret === 'secret' || secret === 'password') {
      throw new Error(
  ...
  validateConfiguration() {
    ...
    if (this.config.validation.maxFileSize > 50 * 1024 * 1024) { // 50MB
      console.warn('File upload limit is very high (>50MB). Consider reducing for security.');
    }
...
`;
  ...
  encrypt(text, key = null) {
    if (!key) {
      key = crypto.scryptSync(this.config.jwt.secret, 'salt', this.config.encryption.keyLength);
    }
