...
class ErrorHandler {
  ...
  handleError(error, context = {}) {
    ...
    return this.generateErrorResponse(classifiedError, handlingStrategy, context);
  }
  ...
  determineHandlingStrategy(error) {
    const strategies = {
      ...
      [this.errorCategories.AI_SERVICE]: {
        ...
        includeDetails: false,
        alerting: false,
        userMessage: 'AI service temporarily unavailable. Using fallback processing.',
        statusCode: 503
      },
  ...
  isSecurityError(message) {
    const securityKeywords = [
      'sql injection', 'xss', 'csrf', 'unauthorized', 'suspicious',
      'malicious', 'attack', 'breach', 'vulnerability', 'dangerous sql'
    ];
  ...
  isAuthError(message) {
    const authKeywords = [
      'authentication', 'token', 'invalid token', 'expired', 'credentials',
  ...
  isDatabaseError(message) {
    const dbKeywords = [
      'database', 'connection', 'query', 'sql', 'transaction',
      'deadlock', 'timeout', 'constraint', 'table', 'column'
    ];
  ...
  isAIServiceError(message) {
    const aiKeywords = [
      'ollama', 'ai service', 'model', 'inference', 'prompt',
      'generation', 'classification', 'intent', 'llm'
    ];
  ...
  sanitizeContext(context) {
    ...
    return securityConfig.sanitizeForLogging ?
      securityConfig.sanitizeForLogging(sanitized) : sanitized;
  }
  ...
  sendAlert(error, context) {
    ...
    console.error(`🚨 CRITICAL ERROR ALERT: ${error.category} - ${error.message}`);
